...
public class ContainerListener implements PathChildrenCacheListener {
	...
	public ContainerListener(ZooKeeperConnection zkConnection,
		...
		this.departingContainerModuleRedeployer = new DepartingContainerModuleRedeployer(zkConnection,
				containerRepository,
				streamFactory, jobFactory, streamDeployments, jobDeployments,
				moduleDeploymentRequests, containerMatcher, stateCalculator);
	...
	public void childEvent(CuratorFramework client, PathChildrenCacheEvent event) throws Exception {
		...
		switch (event.getType()) {
			case CHILD_ADDED:
				this.arrivingContainerModuleRedeployer.redeployModules(client, getContainer(event.getData()));
				break;
			case CHILD_UPDATED:
				break;
			case CHILD_REMOVED:
				this.departingContainerModuleRedeployer.redeployModules(client, getContainer(event.getData()));
				break;
